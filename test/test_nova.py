...
def test_delegation_to_send_rpc():
    ...
    with mock.patch('nameko.sending.send_rpc') as send_rpc:
        ...
        send_rpc.assert_called_with(
            conn, context=ctx, exchange=exchange, topic=topic, method=method,
            args=args, timeout=timeout)

        nova.cast(
            connection=conn, context=ctx, topic=topic, msg=msg,
            options=options)

        send_rpc.assert_called_with(
            conn, context=ctx, exchange=exchange, topic=topic, method=method,
            args=args, noreply=True)

        nova.fanout_cast(connection=conn, context=ctx, topic=topic, msg=msg)

        send_rpc.assert_called_with(
            conn, context=ctx, exchange=None, topic=topic, method=method,
            args=args, fanout=True)
