...
def test_consume_provider(empty_config):
    ...
    consume_provider = Consumer(
        queue=foobar_queue, requeue_on_error=False).bind("name", container)
...
def test_publish_to_exchange(empty_config, maybe_declare, patch_publisher):
    ...
    publisher = Publisher(exchange=foobar_ex).bind("publish", container)
...
def test_publish_to_queue(empty_config, maybe_declare, patch_publisher):
    ...
    publisher = Publisher(queue=foobar_queue).bind("publish", container)
...
def test_consume_from_rabbit(rabbit_manager, rabbit_config):
    ...
    consumer = Consumer(
        queue=foobar_queue, requeue_on_error=False).bind(
