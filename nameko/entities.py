...
def get_reply_exchange(msgid, channel=None):
    return Exchange(
        name=msgid, channel=channel, type='direct', durable=False,
        auto_delete=True)
...
def get_reply_queue(msgid, channel=None):
    ...
    return Queue(
        name=msgid, channel=channel, exchange=exchange, routing_key=msgid,
        durable=False, auto_delete=True, exclusive=True)


def get_topic_exchange(exchange_name, channel=None):
    return Exchange(
        name=exchange_name, channel=channel, type='topic',
        durable=DURABLE_QUEUES, auto_delete=False)
...
def get_topic_queue(exchange_name, topic, channel=None):
    ...
    return Queue(
        ...
        durable=DURABLE_QUEUES, auto_delete=False, exclusive=False)


def get_fanout_exchange(topic, channel=None):
    return Exchange(
        name='{}_fanout'.format(topic), channel=channel, type='fanout',
...
def get_fanout_queue(topic, channel=None, uidgen=None):
    ...
    return Queue(
        name='{}_fanout_{}'.format(topic, unique), channel=channel,
        exchange=exchange, routing_key=topic, durable=False, auto_delete=True,
        exclusive=True)
